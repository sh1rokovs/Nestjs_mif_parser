...
export function fillObj(obj: any, fun: string, ...args: RegExpMatchArray[]): void{
    ...
    if(fun == "PLINE MULTIPLE") {
        ...
        while(true) {
            ...
            const numberLine: number = sString.match(new RegExp('\\n', 'i')).index + 1
            const fileLength: number = sString.length
            const funMatch: RegExpMatchArray = sString.substring(0, numberLine).match(/^\d[\r\n]{1,}/i)

            if(funMatch) {
                const coords: RegExpMatchArray = sString.match(new RegExp('^\\d[\\n\\r]{1,}' + '(.*[\\r\\n]{1,2})'.repeat(Number(funMatch)), 'im'))
                elements[elements.length - 1].coords.push([])
                for(let el = 1; el < coords.length; el++) {
                    const coord: string[] = coords[el].replace("\r\n", "").split(" ")
                    elements[elements.length - 1].coords[elements[elements.length - 1].coords.length - 1].push({
                        "x": Number(coord[0]),
                        "y": Number(coord[1])
                    })
                }
    ...
    if(fun == "TEXT") {
        ...
        if(args[1] != null) {elements[elements.length - 1].angle = Number(args[1][2])}
    }
}
